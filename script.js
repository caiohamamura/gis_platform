...
Vue.createApp({
    setup() {
        ...
        function toggleAccordion(s, groupName) {
            if (s.target.classList.contains("title")) {
                oldOpened = s.target.classList.contains("active");
                layersRaw.value.filter(e => e.group == groupName)["active"] = !oldOpened;
                window.layers = layersRaw.value;
            }
        ...
        onMounted(async () => {
            ...
            layersRaw.value = layers;



            layersRef.value = layers.map(e => e.layers ? e.layers : e).flat();
            const initLayer = layers[0].layers[0];

            activeLayer.value = initLayer.id;
            ...
            for (const layer of layersRef.value) {
                lyr =
                    L.tileLayer(
                        `${layer.location}/{z}/{x}/{y}.png`,
                        {
                            tms: 0,
    ...
    `;
            ...
            for (let layer of layersRef.value) {
                ...
                //     }
                // }
                overleafLayersTitle[layer.title] = overleafLayers[layer.id];
            }
            ...
            let acordionear = function () {
                let labels = Array.from(layerControl2._baseLayersList.querySelectorAll("label"));
                let uniqueGroups = layers.map(e => e.group);

                let groups = {}
                layers.forEach(element => {
                    groups[element.group] = element.layers
                });
                let groupsElement = {};
                let first = true;
                layers.filter(e => e.group).forEach(ii => {
                    let groupName = ii.group;
                    ...
                    layerControl2._baseLayersList.children[0].insertAdjacentElement("afterend", accordion);
                    ii.layers.forEach(ll => {
                        let label = labels.filter(e => e.innerText == ll.title)[0];
